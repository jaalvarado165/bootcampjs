    ...
    formulario.addEventListener("submit", async function (e) {
        ...
        if (response.ok && (result.status === 201 || result.status === 200)) {
            // Registrar también en la tabla persons para que pueda enviar mensajes
            try {
            ...
            // Si falla el registro de persona, solo lo registramos en consola
            // pero no impedimos que el usuario se registre
            if (!personResponse.ok) {
                console.warn("No se pudo crear el registro en persons, pero el usuario fue creado exitosamente");
            }
            } catch (personError) {
